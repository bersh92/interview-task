...
class StepHelper:
    ...
    def hover_then_click(self, hover_locator, click_locator, pause_before_click=0.5):
        # Hovers over one element and then clicks on another element.
        hover_element = WebDriverWait(self.wd, 10).until(
            EC.visibility_of_element_located((self.get_how(hover_locator), hover_locator))
        )
        ActionChains(self.wd).move_to_element(hover_element).pause(0.5).perform()

        click_element = WebDriverWait(self.wd, 10).until(
            EC.element_to_be_clickable((self.get_how(click_locator), click_locator))
        )
        ActionChains(self.wd).move_to_element(click_element).pause(pause_before_click).click().perform()
